...
import { corporateDetails, corporateUpdate, corporatePasswordUpdate, providerGalleryUpdate, providerGalleryRemoveUpdate } from "../../redux/actions/corporate/authAction";
...
const CorporateAccount = () => {
    ...
    useEffect(() => {
        if (!_.isEmpty(corporateData)) {
            ...
            updateData.phone = corporateData.telephone
            updateCState(
                updateData
            )
        }
        if (!_.isEmpty(corporateData)) {
            ...
            updateData.autoLocation = corporateData.location
            updateData.number = corporateData.number
            updateData.street = corporateData.street
            updateData.autoCity = corporateData.city
            updateData.autoPostalCode = corporateData.postalCode
            updateData.autoProvince = corporateData.province
            updateData.autoCountries = corporateData.country
            updateAddState(
                updateData
            )
        }
        setSelectedImagePreview({ file: corporateData.imageGallery })
    ...
    let handleSubmit = async (event) => {
        ...
        if (formIsValid && formAddressIsValid) {
            ...
            if(!autoLatitude && !autoLongitude) {
                ...
                dispatch(corporateUpdate(data)).then(res => {
                    if (res.code == 200) {
                        toast.success(res.message, {
                            position: toast.POSITION.TOP_RIGHT
                        });
    ...
    let handlePasswordSubmit = (event) => {
        ...
        if (formIsValid) {
            ...
            dispatch(corporatePasswordUpdate(data)).then(res => {
                if (res.code == 200) {
                    toast.success(res.message, {
                        position: toast.POSITION.TOP_RIGHT
                    });
