...
import { upcomingBookings, ongoingBookings, completedBookings, AcceptBooking, ratingCreate } from "../../redux/actions/corporate/providerAction";
...
const CorporateMyBookingGarage = () => {
    ...
    const handleAccept = (id) => {
        ...
        dispatch(AcceptBooking(data)).then(res => {
            ...
            setTimeout(
                function() {
                    navigate("/corporate/my-bookings/garages")
                }
                .bind(this),
                1000
            );
                                ...
                                { requestGarageOngoingData && Object.keys(requestGarageOngoingData).length>0 ? requestGarageOngoingData.map( (item, i) =>
                                        <div className="GaragesBox">
                                            ...
                                            <div className="GaragesMiddle">
                                                ...
                                                <aside>
                                                    {  item.corporateVehicleData ?  <p>Vehicle: <span> { item.vehicleData.vehicleNumber } { item.vehicleData.model } { item.vehicleData.make } { item.vehicleData.year } { item.vehicleData.type } </span></p> : '' }
                                                </aside>
                                            </div>
...
export default CorporateMyBookingGarage