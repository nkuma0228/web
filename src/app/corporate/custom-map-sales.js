...
import { salesMarkerList } from "../../redux/actions/corporate/searchAction";
...
const MapContainer = () => {
    ...
    useEffect(() => {
      var markerArray = [];
      if(salesMarkerData && salesMarkerData.length>0) {
          salesMarkerData.filter((item) => {
          ...
          <GoogleMap
           ...
           }
           {
              ...
              (
                ...
                >
                <p><Link to="/corporate/service-auto-sales/detail" className="views markerlocation" target="_blank" onClick={() => handleDetail(selected.id)}>Business Name: {selected.business} <br/> Location: {selected.name}</Link></p>
                </InfoWindow>
              )
            }
       </GoogleMap>
