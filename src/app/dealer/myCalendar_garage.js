...
import { requestGarageCalendar, requestStatusChange } from "../../redux/actions/provider/dealerAction";
...
const MyDealerCalendar = () => {
    ...
    const { requestDealerDataGarageCalendar } = getListData

    useEffect(() => {
        dispatch(requestGarageCalendar())
    },[])
                    ...
                    <div className="TableList">
                        <table style={{width:"125%"}}>
                            <tbody>
                                ...
                                { requestDealerDataGarageCalendar && requestDealerDataGarageCalendar.length>0 ? requestDealerDataGarageCalendar.map((item, i)=>
                                        <tr key={i}>
                                            <td> {i+1} </td>
                                            ...
                                            <td>
                                                <strong>{ t('vendor.scheduling.Name') } : </strong> {item.garageData.firstName } {item.garageData.lastName }, <br /> 
                                                <strong>{ t('vendor.scheduling.Phone') } : </strong> {item.garageData.mobile }, <br /> 
                                                <strong>{ t('vendor.scheduling.Email') } : </strong> {item.garageData.email }
                                            </td>
