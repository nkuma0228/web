...
const SubmitCorporateQuote = () => {
    ...
    const getPartsIndexWise = JSON.parse(window.localStorage.getItem('partsCorporateIndexWise'));

    const [quoteDetails, setQuoteDetails] = useState({})
    const [partsIndexWise, setPartsIndexWise] = useState(getPartsIndexWise)
    
    useEffect(() => {
        const getQuoteDetails = JSON.parse(window.localStorage.getItem('quoteCorporateDetails'));
    ...
    const handleQuoteSubmit = (event) => {
        ...
        dispatch(requestAvailableStatus(data)).then(res => {
            if (res.code == 200) {
                ...
                setTimeout(
                    function() {
                        navigate("/dealer/dashboard/corporate")
                    }
                    .bind(this),
                    2000
                );
...
export default SubmitCorporateQuote;