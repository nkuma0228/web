...
import { requestCorporatePending, requestCorporateAvailable, requestCorporateRejected, requestRejectedStatus } from "../../redux/actions/provider/dealerAction";

const DealerCorporateDashboard = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { i18n, t } = useTranslation();

    const getListData = useSelector(state => state.dealerRequestData)
    const { requestDealerCorporateDataPending, requestCorporateDataAvailable, requestDealerDataReject } = getListData
    ...
    const handleQuotesNavigate = (item) => { 
        window.localStorage.setItem('quoteCorporateDetails', JSON.stringify(item))
        ...
        window.localStorage.setItem('partsCorporateIndexWise', JSON.stringify(partsIndexWise))
        navigate('/dealer/corporate_quote/submit');
    };
    ...
    const handleQuoteView = (item) => {
        window.localStorage.setItem("quoteCorporateDetails", JSON.stringify(item))
        navigate("/dealer/corporate_quote/view")
    }
            ...
            <div className="WrapperArea">
                <div className="WrapperBox">
                    ...
                    <div className="TitleBox">
                        <h4> { t('vendor.dashboard.CorporateQuote') } </h4>
                    </div>
                                    ...
                                    <table>
                                        <thead>
                                            <tr>
                                                ...
                                                <th> { t('vendor.dashboard.CorporateDetails') } </th>
                                        ...
                                        <tbody>
                                         {
                                            requestDealerCorporateDataPending && requestDealerCorporateDataPending.length>0 ? requestDealerCorporateDataPending.map((item , i) =>
                                        
                                                    <tr key={i}>
                                                        <td> {i+1} </td>
                                                        <td> {item.vehicleData.make } {item.vehicleData.model } </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <strong>{ t('vendor.dashboard.Name') } : </strong> {item.corporateData.firstName } {item.corporateData.lastName }, <br /> 
                                                            <strong>{ t('vendor.dashboard.Phone') } : </strong> {item.corporateData.mobile }, <br /> 
                                                            <strong>{ t('vendor.dashboard.Email') } : </strong> {item.corporateData.email }
                                                        </td>
                            ...
                            <div className="tab-pane fade" id="Submitted">
                                ...
                                <div className="TableList">
                                    <table style={{ width: "115%" }}>
                                        ...
                                        {
                                            requestCorporateDataAvailable && requestCorporateDataAvailable.length>0 ? requestCorporateDataAvailable.map((item , i) =>
...
export default DealerCorporateDashboard