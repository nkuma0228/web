...
import { requestCorporateCalendar, requestCorporateStatusChange, requestMechanicAssign } from "../../redux/actions/provider/garageAction";

const CorporateScheduling = () => {
    ...
    useEffect(() => {
        dispatch(requestCorporateCalendar())
    },[])
    useEffect(() => {
    ...
    const handleDetail = (id) => {
        ...
        navigate("/garage/mycalendar/corporate/detail")
    }

    const inputHandleOption = (status, id) => {
        
        const data = { id:id, status:status };
        dispatch(requestCorporateStatusChange(data)).then(res => {
            if (res.code == 200) {
                                ...
                                { requestDataCalendar && requestDataCalendar.length>0 ? requestDataCalendar.map((item, i)=>
                                        <tr key={i}>
                                            ...
                                            <td>
                                                ...
                                                <smap>
                                                    <Link className="Blue" to="/garage/mycalendar/corporate/detail" onClick={()=>handleDetail(item._id)}>
...
export default CorporateScheduling